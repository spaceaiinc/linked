...
import LinkedInInputCapture from '@/components/input/LinkedinInput'
...
export default function Page() {
  ...
  const InfoCard = (
    <AppInfo title="使い方" background="bg-accent/10">
      ...
      <ul className="mt-4 ml-4 text-sm space-y-2 flex flex-col mb-4 relative xs:leading-7">
        <li className="text-l flex">
          ...
          <span className="ml-2">
            「連携する」ボタンをクリックして、LinkedInアカウントと紐づける。
          </span>
        </li>
  ...
  //     // Wait for user to login and get cookies
  //     if (linkedInWindow) {
  //       // Create an interval to check if user has logged in
  //       const checkInterval = setInterval(async () => {
  //         try {
  //           // Try to get LinkedIn cookies
  //           const response = await fetch('/api/apps/linkedin/auth/cookie', {
  //             method: 'POST',
  //           })
